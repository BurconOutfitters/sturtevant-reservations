...
if( !class_exists( 'DEXBCCF_SalesForce' ) )
{
    class DEXBCCF_SalesForce extends DEXBCCF_BaseAddon
    {
		...
		public function get_addon_form_settings( $form_id )
		{
			...
			if( 
				isset( $_REQUEST[ 'dexbccf_salesforce_oid' ] )
			)
			{
				$data = array();
			...
			<div id="metabox_basic_settings" class="postbox" >
				...
				<div class="inside"> 
					<table cellspacing="0">
						<tr>
							<td style="white-space:nowrap;width:200px;"><?php _e('Organizational ID', 'bccf');?>:</td>
							<td><input type="text" name="dexbccf_salesforce_oid" value="<?php echo esc_attr( $oid ); ?>" ></td>
						</tr>
						<tr>
							<td style="white-space:nowrap;width:200px;"><?php _e('Enabling debug', 'bccf');?>:</td>
							<td><input type="checkbox" name="dexbccf_salesforce_debug" <?php echo ( ( $debug ) ? 'CHECKED' : '' ); ?> ></td>
						</tr>
						<tr>
							<td style="white-space:nowrap;width:200px;"><?php _e('Debug email', 'bccf');?>:</td>
							<td><input type="text" name="dexbccf_salesforce_debug_email" value="<?php echo esc_attr( $debugemail ); ?>" ></td>
						</tr>
						<tr><td colspan="2"><strong><?php _e('Lead Attributes', 'bccf');?>:</strong></td></tr>
						<tr>
							<td colspan="2">
								<table>
									<?php
									...
									<tr>
										<td colspan="2">
											<input type="button" value="<?php esc_attr_e('Add attribute', 'bccf');?>" onclick="dexbccf_salesforce_addAttr( this );" />
										</td>
									</tr>
								</table>
							</td>	
        ...
        {
			$this->description = __("The add-on allows create SalesForce leads with the submitted information", 'bccf' );
			...
			// Export the lead
			add_action( 'dexbccf_process_data', array( &$this, 'export_lead' ) );
        } // End __construct
        ...
        {
			wp_enqueue_style( 'dexbccf_salesforce_addon_css', plugins_url('/salesforce.addon/css/styles.css', __FILE__ ) );
			wp_enqueue_script( 'dexbccf_salesforce_addon_js', plugins_url('/salesforce.addon/js/scripts.js',  __FILE__), array( 'jquery', 'jquery-ui-autocomplete' ) );
				...
				if( $row && !empty( $row->oid ) )
				{
					...
					$args = array(
						...
						'headers' 	=> array(
							...
							'user-agent' => 'WordPress-to-Lead for booking-calendar-contact-form plugin - WordPress/'.$wp_version.'; '.get_bloginfo( 'url' ),
						),
						'timeout' => 45,
						'sslverify'	=> false,
					);
