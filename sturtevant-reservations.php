...
function dex_bccf_get_public_form( $pages = '' ) {
    ...
    } elseif ( defined( 'DEX_BCCF_CALENDAR_FIXED_ID' ) ) {
    ...
    if ( $previous_label == '' ) {
        $previous_label = __( 'Previous', 'sc-res' );
    } else {
    ...
    if ( DEX_BCCF_DEFAULT_DEFER_SCRIPTS_LOADING ) {
        ...
        $dependencies = [
            ...
            'jquery-ui-tooltip',
            'query-stringify',
            'cp_contactformpp_validate_script',
            'cp_contactformpp_rcalendar'
        ];
    ...
    if ( $button_label == '' ) {
        $button_label = __( 'Continue', 'sc-res' );
    } else {
...
function set_dex_bccf_insert_button() {
    print '<a href="javascript:send_to_editor(\'[CP_BCCF_FORM]\')" title="' . __( 'Insert Booking Calendar', 'sc-res' ) . '"><img hspace="5" src="' . plugins_url( '/images/dex_apps.gif', __FILE__ ) . '" alt="' . __( 'Insert Booking Calendar', 'sc-res' ) . '" /></a>';
}
    ...
    if ( isset( $_GET['page'] ) && $_GET['page'] == 'dex_bccf' ) {
        ...
        wp_enqueue_script(
            ...
            [
                ...
                'query-stringify',
                'cp_contactformpp_rcalendar',
                'cp_contactformpp_rcalendaradmin'
            ]
        );
...
function dex_bccf_get_services() {
    ...
    for ( $k = 1; $k <= DEX_BCCF_DEFAULT_SERVICES_FIELDS; $k++ ) {
        ...
        foreach ( $services as $item ) {
            if ( trim( $item ) != '' ) {
                $dex_buffer[$k] .= '<option value="' . esc_attr( $item ) . '">' . __( trim( substr( $item, strpos( $item, '|' ) +1 ) ), 'sc-res' ) . '</option>';
            }
        }
...
function dex_bccf_export_iCal() {
    ...
    if ( ! ( $_GET['id'] != '' && substr( md5( $_GET['id'] . $_SERVER['DOCUMENT_ROOT'] ), 0, 10 ) == $_GET['verify'] ) ) {
        echo __( 'Access denied - verify value is not correct.', 'sc-res' );
        exit;
    }
...
function dex_bccf_check_posted_data() {
    ...
    if ( ! isset( $_GET['hdcaptcha_dex_bccf_post'] ) || $_GET['hdcaptcha_dex_bccf_post'] == '' ) {
        $_GET['hdcaptcha_dex_bccf_post'] = @$_POST['hdcaptcha_dex_bccf_post'];
    }

    if ( ! apply_filters( 'dexbccf_valid_submission', true ) || (
    ...
    if ( $option_calendar_enabled != 'false' ) {
        ...
        // Calculate days.
        $days = round( ( strtotime( $_POST['dateAndTime_e'] ) - strtotime( $_POST['dateAndTime_s'] ) ) / ( 24 * 60 * 60 ) );
    ...
    if ( count( $codes ) ) {
        ...
        $discount_note = ' (' . $coupon->discount . '% ' . __( 'discount applied', 'sc-res' ) . ')';
    }
    ...
    $buffer_A .= ( $services_text != '' ? $services_text . "\n\n" : '' ) .
                 ( $coupon ? "\n" . __( 'Coupon code: ', 'sc-res' ) . $coupon->code . $discount_note . "\n\n" : '' );

    if ( $price != 0 ) {
        $buffer_A .= 'Total Cost: ' . dex_bccf_get_option( 'currency', DEX_BCCF_DEFAULT_CURRENCY ) . ' ' . $price . "\n\n";
    }

    $buffer = $_POST['selMonth_start' . $selectedCalendar_sfx] . '/' . $_POST['selDay_start' . $selectedCalendar_sfx] . '/' . $_POST['selYear_start' . $selectedCalendar_sfx] . '-' . $_POST['selMonth_end' . $selectedCalendar_sfx] . '/' . $_POST['selDay_end' . $selectedCalendar_sfx] . '/' . $_POST['selYear_end' . $selectedCalendar_sfx] . "\n" . $buffer_A . "*-*\n";
    ...
    if ( ! $rows_affected ) {
        echo __( 'Error saving data! Please try again.', 'sc-res' );
        echo '<br /><br />' . __( 'Error debug information: ', 'sc-res' ) . mysql_error();
        exit;
    }
    ...
    if ( floatval( $price ) > 0 && dex_bccf_get_option( 'enable_paypal', DEX_BCCF_DEFAULT_ENABLE_PAYPAL ) == '3' ) {
        ...
        header( 'Location: https://www.beanstream.com/scripts/payment/payment.asp?merchant_id=' . dex_bccf_get_option( 'enable_beanstream_id', '' ) . '&trnOrderNumber=' . $item_number . '&trnAmount=' . $price . '&approvedPage=' . urlencode( cp_bccf_get_FULL_site_url() . '/?dex_bccf=ipn&ipncheck=1&itemnumber=' . $item_number . '&beanstrean=1' ) . '&declinedPage=' . urlencode( dex_bccf_get_option( 'url_cancel', DEX_BCCF_DEFAULT_CANCEL_URL) )
        );

       exit;

    } elseif ( floatval( $price ) > 0 && dex_bccf_get_option( 'enable_paypal', DEX_BCCF_DEFAULT_ENABLE_PAYPAL ) && ( ! $paypal_optional || ( @$_POST['bccf_payment_option_paypal'] == '1' ) ) ) {
?>
<html>
    <head>
        <title><?php echo esc_html__( 'Redirecting to Paypal&hellip', 'sc-res' ); ?></title>
    </head>
    <body>
...
<?php exit();
        ...
        if ( $redirect ) {
            ...
            header( __( 'Location: ', 'sc-res' ) . $location );
            exit;
        }
    }
...
function dex_bccf_caculate_price_overall( $startday, $enddate, $calendar, $default_price, $services_formatted ) {
    ...
    */
    ...
    $days = round( ( $enddate - $startday ) / ( 24 * 60 * 60 ) );
    if ( dex_bccf_get_option( 'calendar_mode', DEX_BCCF_DEFAULT_CALENDAR_MODE ) == 'false' ) {
        $days++;
    }
...
function dex_bccf_caculate_price( $startday, $enddate, $calendar, $default_price ) {
    ...
    while ( ( ( $enddate>$startday ) && ! $mode ) || ( ( $enddate>=$startday ) && $mode ) ) {
    ...
    if ( trim( @$season_prices[count( $season_prices ) - 1][$days] )
        &&
        // Get higher price if different seasons.
        floatval( $price ) < floatval( trim( @$season_prices[count( $season_prices ) - 1][$days] ) ) )
 ...
 * @return void
 */
function dex_bccf_load_discount_codes() {    global $wpdb;

    // Prevent loading coupons from outside admin area.
    if ( ! current_user_can( 'edit_pages' ) ) {
        echo __( 'No enough privilegies to load this content.', 'sc-res' );
        exit;
    }
    ...
    if ( isset( $_GET['add'] ) && $_GET['add'] == '1' ) {
        $wpdb->insert(
            DEX_BCCF_DISCOUNT_CODES_TABLE_NAME,
            array(
    ...
    if ( count ( $codes ) ) {
        echo '<table>';
        echo '<tr>';
        echo '  <th style="padding:2px;background-color: #cccccc;font-weight:bold;">' . __( 'Cupon Code', 'sc-res' ) . '</th>';
        echo '  <th style="padding:2px;background-color: #cccccc;font-weight:bold;">' . __( 'Discount %', 'sc-res' ) . '</th>';
        echo '  <th style="padding:2px;background-color: #cccccc;font-weight:bold;">' . __( 'Valid until', 'sc-res' ) . '</th>';
        echo '  <th style="padding:2px;background-color: #cccccc;font-weight:bold;">' . __( 'Options', 'sc-res' ) . '</th>';
        echo '</tr>';
        foreach ( $codes as $value ) {
           echo '<tr>';
           ...
           echo '   <td>[<a href="javascript:dex_delete_coupon(' . $value->id . ')">' . __( 'Delete', 'sc-res' ) . '</a>]</td>';
           echo '</tr>';
        }
        echo '</table>';
    } else {
        echo __( 'No discount codes listed for this calendar yet.', 'sc-res' );
    }
...
function dex_bccf_load_season_prices() {
    ...
    if ( isset( $_GET['add'] ) && $_GET['add'] == '1' ) {
        $wpdb->insert(
            $wpdb->prefix . DEX_BCCF_SEASON_PRICES_TABLE_NAME_NO_PREFIX,
            array(
                'cal_id'    => CP_BCCF_CALENDAR_ID,
                'price'     => $_GET['price'],
    ...
    if ( count ( $codes ) ) {
        ...
        echo __( 'No season prices listed for this calendar yet.', 'sc-res' );
    }
...
function dex_bccf_check_IPN_verification() {
    ...
    if ( @$_GET['beanstrean'] != '1' ) {
        ...
        header( __( 'Location: ', 'sc-res' ) . dex_bccf_get_option( 'url_ok', DEX_BCCF_DEFAULT_OK_URL ) );
    }
...
function dex_process_ready_to_go_bccf( $itemnumber, $payer_email = '', $params ) {
    ...
    $email_subject1 = __( dex_bccf_get_option('email_subject_confirmation_to_user', DEX_BCCF_DEFAULT_SUBJECT_CONFIRMATION_EMAIL), 'sc-res' );
    $email_content1 = __( dex_bccf_get_option('email_confirmation_to_user', DEX_BCCF_DEFAULT_CONFIRMATION_EMAIL), 'sc-res' );
    $email_subject2 = __( dex_bccf_get_option('email_subject_notification_to_admin', DEX_BCCF_DEFAULT_SUBJECT_NOTIFICATION_EMAIL), 'sc-res' );
    $email_content2 = __( dex_bccf_get_option('email_notification_to_admin', DEX_BCCF_DEFAULT_NOTIFICATION_EMAIL), 'sc-res' );

    $option_calendar_enabled = dex_bccf_get_option( 'calendar_enabled', DEX_BCCF_DEFAULT_CALENDAR_ENABLED );
    if ( $option_calendar_enabled != 'false' ) {
        $information = __( 'Item: ', 'sc-res' ) . $mycalendarrows[0]->uname . "\n\n" . __( 'Date From-To: ', 'sc-res' ) . $myrows[0]->booked_time_s . " - " . $myrows[0]->booked_time_e . "\n\n" . $myrows[0]->question;
    } else {
        $information = __( 'Item: ', 'sc-res' ) . $mycalendarrows[0]->uname . "\n\n" . $myrows[0]->question;
    }
    ...
    $rows_affected = $wpdb->insert(
        TDE_BCCFCALENDAR_DATA_TABLE,
        [
            'reservation_calendar_id' => $myrows[0]->calendar,
            ...
            'title'                   => ( $_POST[$to] ? $_POST[$to] : __( 'Booked', 'sc-res' ) ),
    ...
    // Send email to user.
    if ( 'html' == $mycalendarrows[0]->copyuser_emailformat ) {
    ...
    // Send email to admin.
    if ( 'html' == $mycalendarrows[0]->notification_emailformat ) {
        $content_type = "Content-Type: text/html; charset=utf-8\n";
    } else {
        $content_type = "Content-Type: text/plain; charset=utf-8\n";
    }
...
function dex_bccf_save_options() {
    ...
    $wpdb->update ( DEX_BCCF_CONFIG_TABLE_NAME, $data, [ 'id' => CP_BCCF_CALENDAR_ID ] );
...
function dex_bccf_appointments_export_csv() {
    ...
    if ( CP_BCCF_CALENDAR_ID != 0 ) {
        $cond .= " AND calendar=".CP_BCCF_CALENDAR_ID;
    }

    $events_query = "SELECT ".DEX_BCCF_TABLE_NAME.".*,".DEX_BCCF_CONFIG_TABLE_NAME.".uname FROM ".DEX_BCCF_TABLE_NAME." INNER JOIN ".DEX_BCCF_CONFIG_TABLE_NAME." ON ".DEX_BCCF_TABLE_NAME.".calendar=".DEX_BCCF_CONFIG_TABLE_NAME.".id INNER JOIN  ".DEX_BCCF_CALENDARS_TABLE_NAME." on  ".DEX_BCCF_TABLE_NAME.".id=".DEX_BCCF_CALENDARS_TABLE_NAME.".reference  "." WHERE 1=1 ".$cond." ORDER BY `datatime_s` DESC";
    ...
    $fields       = [ "Calendar ID", "Calendar Name" /**, "Time Start", "Time End"*/ ];
    $values       = [];

    foreach ( $events as $item ) {

        $value = [ $item->calendar, $item->uname /**, $item->booked_time_s, $item->booked_time_e*/ ];
        $data  = [];
...
function dex_bccf_check_reminders() {
    ...
    $query = "SELECT nremind_emailformat,notification_from_email,reminder_subject,reminder_content,uname,".TDE_BCCFCALENDAR_DATA_TABLE.".* FROM ".TDE_BCCFCALENDAR_DATA_TABLE." INNER JOIN ".DEX_BCCF_CONFIG_TABLE_NAME." ON ".TDE_BCCFCALENDAR_DATA_TABLE.".reservation_calendar_id=".DEX_BCCF_CONFIG_TABLE_NAME.".id "." WHERE enable_reminder=1 AND (reminder='0' OR reminder='') AND datatime_s<DATE_ADD(now(),INTERVAL reminder_hours HOUR) AND datatime_s>'" . date( 'Y-m-d H:i:s' ) . "'";
    ...
    foreach ( $apps as $app ) {
        ...
        // Send email.
        if ( 'html' == $app->nremind_emailformat ) {
            $content_type = "Content-Type: text/html; charset=utf-8\n";
        } else {
            $content_type = "Content-Type: text/plain; charset=utf-8\n";
        }
        ...
        wp_mail(
            $app_source[0]->notifyto,
            $app->reminder_subject,
            $email_content,
    ...
    $query = "SELECT nremind_emailformat2,notification_from_email,reminder_subject2,reminder_content2,uname,".TDE_BCCFCALENDAR_DATA_TABLE.".* FROM ".TDE_BCCFCALENDAR_DATA_TABLE." INNER JOIN ".DEX_BCCF_CONFIG_TABLE_NAME." ON ".TDE_BCCFCALENDAR_DATA_TABLE.".reservation_calendar_id=".DEX_BCCF_CONFIG_TABLE_NAME.".id "." WHERE enable_reminder=1 AND (reminder='0' OR reminder='' OR reminder='1') AND datatime_e<DATE_SUB(now(),INTERVAL reminder_hours2 HOUR)"; // AND datatime_e>'".date("Y-m-d H:i:s")."'";
    ...
    foreach ( $apps as $app ) {
        ...
        // Send email.
        if ( 'html' == $app->nremind_emailformat2 ) {
            $content_type = "Content-Type: text/html; charset=utf-8\n";
        } else {
            $content_type = "Content-Type: text/plain; charset=utf-8\n";
        }
...
function dex_bccf_calendar_ajaxevent() {
    ...
    switch ( $_GET['dex_bccf_calendar_load2'] ) {
        ...
        case 'add':
            if ( ! current_user_can( 'edit_pages' ) ) {
                $ret['isSuccess'] = false;
                $ret['msg'] = __( 'Permissions error: No enough privilegies to add an item.', 'sc-res' );
            } else {
        ...
        case 'edit':
            if ( ! current_user_can( 'edit_pages' ) ) {
                $ret['isSuccess'] = false;
                $ret['msg'] = __( 'Permissions error: No enough privilegies to edit an item.', 'sc-res' );
            } else {
        ...
        case 'delete':
            if ( ! current_user_can( 'edit_pages' ) ) {
                $ret['isSuccess'] = false;
                $ret['msg'] = __( 'Permissions error: No enough privilegies to delete an item.', 'sc-res' );
            } else {
        ...
        default:
          $ret['isSuccess'] = false;
          $ret['msg']       = __( 'Unknown calendar action: ', 'sc-res' ) . $_GET['dex_bccf_calendar_load2'];
    }
...
function dex_bccf_calendar_add( $ret ) {
    ...
    $wpdb->query( "insert into ".TDE_BCCFCALENDAR_DATA_TABLE."(viadmin,reservation_calendar_id,datatime_s,datatime_e,title,description,color) "." values(1," . esc_sql( $calid ) . ",'" . esc_sql( $_POST['startdate'] ) . "','" . esc_sql( $_POST['enddate'] ) . "','".esc_sql( $_POST['title'] ) . "','" . esc_sql( $_POST['description'] ) . "','" . esc_sql( $_POST['color'] ) . "')" );

    $ret['events'][0] = [
        'id'          => $wpdb->insert_id,
        'dl'          => date( 'm/d/Y', strtotime( $_POST['startdate'] ) ),
        'du'          => date( 'm/d/Y', strtotime( $_POST['enddate'] ) ),
        'title'       => $_POST['title'],
			...
			if ( $entry != '.' && $entry != '..' && is_dir( $tpls_dir->path . '/' . $entry ) && file_exists( $tpls_dir->path . '/' . $entry . '/config.ini' ) ) {
				if ( ( $ini_array = parse_ini_file( $tpls_dir->path . '/' . $entry . '/config.ini' ) ) !== false ) {
					...
					if ( ! empty( $ini_array['file'] ) ) {
                        $ini_array['file'] = plugins_url( 'templates/' . $entry . '/' . $ini_array['file'], __FILE__ );
                    }

					if ( ! empty( $ini_array['thumbnail'] ) ) {
                        $ini_array['thumbnail'] = plugins_url( 'templates/' . $entry . '/' . $ini_array['thumbnail'], __FILE__ );
                    }
...
function dex_bccf_translate_json( $str ) {
    ...
    $form_data[1][0]->title       = __( $form_data[1][0]->title, 'sc-res' );
    $form_data[1][0]->description = __( $form_data[1][0]->description, 'sc-res' );

    for ( $i = 0; $i < count( $form_data[0] ); $i++ ) {

        $form_data[0][$i]->title           = __( $form_data[0][$i]->title, 'sc-res' );
        $form_data[0][$i]->userhelpTooltip = __( $form_data[0][$i]->userhelpTooltip, 'sc-res' );
        $form_data[0][$i]->userhelp        = __( $form_data[0][$i]->userhelp, 'sc-res' );
        ...
        if ( $form_data[0][$i]->ftype == 'fCommentArea' ) {
            ...
            if ( $form_data[0][$i]->ftype == 'fradio' || $form_data[0][$i]->ftype == 'fcheck' || $form_data[0][$i]->ftype == 'fradio' ) {
                for ( $j = 0; $j < count( $form_data[0][$i]->choices ); $j++ ) {
                    $form_data[0][$i]->choices[$j] = __( $form_data[0][$i]->choices[$j], 'sc-res' );
                }
            }
        }
...
function dex_bccf_form_result( $atts, $content = '', $id = 0 ) {
    ...
    if ( $id == 0 && !empty( $_SESSION['cp_cff_form_data'] ) ) {
    ...
    if ( ! empty( $id ) ) {
        ...
        if ( ! is_null( $result ) ) {
            ...
            $atts = shortcode_atts( [ 'fields' => '' ], $atts );
            ...
            if ( empty( $form_data ) ) {
                ...
                $fields['ipaddr']    = $_SERVER['REMOTE_ADDR'];
                // $result->ipaddr; Commented out in the original plugin.
                ...
                foreach ( $atts['fields'] as $field ) {
                    ...
                    if ( isset( $fields[$field] ) ) {
                        ...
                        if ( isset( $result->paypal_post[$field] ) ) {
                            ...
                            if ( is_array( $result->paypal_post[$field] ) ) {
                                $result->paypal_post[$field] = implode( ',', $result->paypal_post[$field] );
                            }

                            $str .= '<p>{$fields[$field]->title} {$result->paypal_post[$field]}</p>';

                        } elseif ( in_array( $fields[$field]->ftype, [ 'fSectionBreak' ] ) ) {
...
function _dex_bccf_extract_tags( $message ) {
	...
	if ( preg_match_all(	"/<%(info|fieldname\d+|fieldname\d+_label|fieldname\d+_shortlabel|fieldname\d+_value|fieldname\d+_url|fieldname\d+_urls|coupon|itemnumber|final_price|payment_option|ipaddress|currentdate_mmddyyyy|currentdate_ddmmyyyy)\b(?:(?!%>).)*%>/i", $message, $matches ) ) {
...
function _dex_bccf_replace_vars( $fields, $params, $message, $buffer = '', $contentType = 'html', $itemnumber = '' ) {
	...
	while ( preg_match( "/<%\s*fieldname(\d+)_block\s*%>/", $message, $matches ) ) {
		...
		if ( empty( $params['fieldname' . $matches[1]] ) ) {
			...
			$message = preg_replace( [ '/<%\s*fieldname' . $matches[1] . '_block\s*%>/', '/<%\s*fieldname' . $matches[1] . '_endblock\s*%>/' ], '', $message );
        }
	...
	foreach ( $params as $item => $value ) {
		...
		if ( isset( $tags[$item] ) ) {
			...
			if ( 'html' == $contentType ) {
				...
				$shortlabel = str_replace( [ '&lt;', '&gt;', '\"', "\'" ], [ '<', '>', '"', "'" ], $shortlabel );
