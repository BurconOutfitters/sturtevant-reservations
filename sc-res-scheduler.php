<?php
/**
 * The scheduler for the calendar system.
 *
...
/**
 * Language constants, translate the following.
 *
 * @since 1.0.0
 */
$l_calendar        = __( 'Calendar', 'sc-res' );
$l_min_nights      = __( 'The minimum number of nights to book is', 'sc-res' );
$l_max_nights      = __( 'The maximum number of nights to book is', 'sc-res' );
$l_select_dates    = __( 'Select start and end dates', 'sc-res' );
$l_p_select        = __( 'Please select start and end dates', 'sc-res' );
$l_select_start    = __( 'Select Start Date', 'sc-res' );
$l_select_end      = __( 'Select End Date', 'sc-res' );
$l_cancel_c        = __( 'Cancel Selection', 'sc-res' );
$l_sucess          = __( 'Successfully', 'sc-res' );
$l_cost            = __( 'Cost', 'sc-res' );
$l_coupon          = __( 'Coupon code (optional)', 'sc-res' );
$l_service         = __( 'Service', 'sc-res' );
$l_sec_code        = __( 'Please enter the security code', 'sc-res' );
$l_sec_code_low    = __( 'Security Code (lowercase letters)', 'sc-res' );
$l_payment_options = __( 'Payment options', 'sc-res' );
$l_continue        = __( $button_label, 'sc-res' );
...
<form class="cpp_form" name="dex_bccf_pform" id="dex_bccf_pform" action="<?php get_site_url(); ?>" method="post" enctype="multipart/form-data" onsubmit="return doValidate(this);">
    ...
    <div <?php echo ( count( $myrows ) < 2 ? 'style="display:none"' : '' ); ?>>
        ...
        foreach ( $myrows as $item ) {
            echo '<option value=' . $item->id . '>' . __( $item->uname, 'sc-res' ) . '</option>';
        }
        echo '</select>';
        ...
        foreach ( $myrows as $item ) {
            ...
            if ( $value != '' ) {
                $value = date( $dformat_php, strtotime( $value ) );
            }
            ...
            $cfmode = $item->calendar_holidaysdays;
            if ( strlen( $cfmode ) != 7 ) {
                $cfmode = '1111111';
            }
            $workingdates = '[' . $cfmode[0] . ',' . $cfmode[1] . ',' . $cfmode[2] . ',' . $cfmode[3] . ',' . $cfmode[4] . ',' . $cfmode[5] . ',' . $cfmode[6] . ']';
            ...
            $cfmode = $item->calendar_startresdays;
            if ( strlen( $cfmode ) != 7 ) {
                $cfmode = '1111111';
            }
            $startReservationWeekly = '[' . $cfmode[0] . ',' . $cfmode[1] . ',' . $cfmode[2] . ',' . $cfmode[3] . ',' . $cfmode[4] . ',' . $cfmode[5] . ',' . $cfmode[6] . ']';
            ...
            $fixrd_l = $item->calendar_fixedreslength;
            if ( $fixrd_l == '' ) {
                $fixrd_l = '1';
            }
        ...
        function dex_do_init( id ) {
            ...
            myjQuery(function() {
                ...
                $calendarjQuery( function() {
                    ...
                    $calendarjQuery( '#calarea' + id ).rcalendar({
                        'calendarId' : id,
    ...
    <?php } ?>
        function doValidate( form ) {
            ...
            $minn = dex_bccf_get_option( 'calendar_minnights', '0' ); if ( $minn == '' ) {
                $minn = '0';
            }
            $maxn = dex_bccf_get_option( 'calendar_maxnights', '0' ); if ( $maxn == '0' || $maxn == '' ) {
                $maxn = '365';
            }
            } ?>
    ...
    <?php }
    echo '<fieldset class="res-cal-submit-deposit"><legend class="res-cal-submit-deposit">' . esc_html__( 'Submit Payment', 'sc-res' ) . '</legend><div id="servcontbccf">';
    ...
    <div class="fields" id="field-c0">
        ...
        <div class="dfield">
            <input type="radio" name="bccf_payment_option_paypal" value="1" vt="1" checked /> <?php echo __( dex_bccf_get_option( 'enable_paypal_option_yes', DEX_BCCF_DEFAULT_PAYPAL_OPTION_YES ), 'sc-res' ); ?><br />
            <input type="radio" name="bccf_payment_option_paypal" value="0" vt="0" /> <?php echo  __( dex_bccf_get_option( 'enable_paypal_option_no',DEX_BCCF_DEFAULT_PAYPAL_OPTION_NO ), 'sc-res' ); ?>
        </div>
        <div class="clearer"></div>
    </div>
    <?php }
