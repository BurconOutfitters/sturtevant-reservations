...
{   
    $myrows = $wpdb->get_results( "SELECT * FROM ".DEX_BCCF_TABLE_NAME." WHERE id=".intval($_GET['lup']) );
    $params = unserialize($myrows[0]->buffered_date);   
    dex_process_ready_to_go_bccf(intval($_GET['lup']), '', $params);    
	 ...
	 <?php for ($i=($current_page-1)*$records_per_page; $i<$current_page*$records_per_page; $i++) if (isset($events[$i])) { ?>
	  <tr class='<?php if (!($i%2)) { ?>alternate <?php } ?>author-self status-draft format-default iedit' valign="top">
		...
		<td>		  		  
          ...
          <input type="button" name="caldelete_<?php echo $events[$i]->id; ?>" value="Mark as paid" onclick="cp_updatePaid(<?php echo $events[$i]->id; ?>);" />                               
		</td>		
      </tr>
     <?php } ?>
	</tbody>
