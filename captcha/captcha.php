...
if ( $_GET['hdwtest'] == 'sessiontest' ) {
    ...
    if ( $_GET['autocall'] != 1 ) {
        ...
        if ( $_SESSION['tmpvar'] != 'ok' ) {
            die( __( 'Session Error', 'sc-res' ) );
        } else {
            die( __( 'Sessions works on your server!', 'sc-res' ) );
        }
	}
...
if ( $_GET['width'] == '' || ! is_numeric( $_GET['width'] ) ) {
	$_GET['width'] = '180';
}

if ( $_GET['height'] == '' || ! is_numeric( $_GET['height'] ) ) {
	$_GET['height'] = '60';
}

if ( $_GET['letter_count'] == ''|| ! is_numeric( $_GET['letter_count'] ) ) {
	$_GET['letter_count'] = '5';
}

if ( $_GET['min_size'] == ''|| ! is_numeric( $_GET['min_size'] ) ) {
	$_GET['min_size'] = '35';
}

if ( $_GET['max_size'] == ''|| ! is_numeric( $_GET['max_size'] ) ) {
	$_GET['max_size'] = '45';
}

if ( $_GET['noise'] == ''|| ! is_numeric( $_GET['noise'] ) ) {
	$_GET['noise'] = '200';
}

if ( $_GET['noiselength'] == ''|| ! is_numeric( $_GET['noiselength'] ) ) {
	$_GET['noiselength'] = '5';
}
...
/*
if ( empty( $_POST['hdcaptcha_dex_bccf_post'] ) || $_POST['hdcaptcha_dex_bccf_post'] != $_SESSION['rand_code'] ) {
