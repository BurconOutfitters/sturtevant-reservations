<?php
/**
 * The admin page for managing forms.
 *
...
sc_res_add_field_verify( SC_RES_CONFIG_TABLE_NAME, 'master', "varchar(50) DEFAULT '0' NOT NULL" );

if ( ! defined( 'CP_BCCF_CALENDAR_ID' ) ) {
	define ( 'CP_BCCF_CALENDAR_ID', intval( $_GET['cal'] ) );
}

global $wpdb, $sc_res_addons_objs_list, $sc_res_addons_active_list;

$mycalendarrows = $wpdb->get_results( 'SELECT * FROM ' . SC_RES_CONFIG_TABLE_NAME . ' WHERE `' . TDE_BCCFCONFIG_ID . '`=' . CP_BCCF_CALENDAR_ID );

if ( 'POST' == $_SERVER['REQUEST_METHOD'] && isset( $_POST['sc_res_post_options'] ) ) {
    echo '<div id="setting-error-settings_updated" class="notice notice-success is-dismissible"> <p><strong>' . esc_html__( 'Settings saved.', 'sc-res' ) . '</strong></p></div>';
}

$current_user = wp_get_current_user();

if ( cp_sc_res_is_administrator() || $mycalendarrows[0]->conwer == $current_user->ID ) :

	$request_costs = explode( ';', sc_res_get_option( 'request_cost', SC_RES_DEFAULT_COST ) );
	if ( ! count( $request_costs ) ) {
		$request_costs[0] = SC_RES_DEFAULT_COST;
	}
...
<div class="wrap reservations reservation-manage">
	<h1><?php _e( 'Reservation Form Settings', 'sc-res' ); ?></h1>
	<p class="description"><?php _e( 'Form fields, availability calendar, rates & prides, etc.', 'sc-res' ); ?></p>
	<p>
		<input class="button" type="button" name="backbtn" value="<?php _e( 'Back to Forms', 'sc-res' ); ?>" onclick="document.location='admin.php?page=reservations';">
	</p>
	<hr />
	<h2><?php echo esc_html__( 'Settings for the ', 'sc-res' ) . $mycalendarrows[0]->uname . esc_html__( 'Form', 'sc-res' ); ?></h2>
	<form method="post" name="dexconfigofrm" action="">
		<table class="form-table">
			<tbody>
				<tr>
					<th scope="row">
						<label for="masteritem"><?php _e( 'Master Calendar', 'sc-res' ); ?></label>
					</th>
					<td>
						<?php
						$value     = sc_res_get_option( 'master', '0' );
						$calendars = $wpdb->get_results( 'SELECT * FROM ' . SC_RES_CONFIG_TABLE_NAME . ' WHERE (master=\'0\' Or master=\'\') AND  `id`<>' . CP_BCCF_CALENDAR_ID );
						?>
		...
		<input name="sc_res_post_options" type="hidden" value="1" />
		...
		<table class="form-table">
			<tbody>
				<tr valign="top">
					...
					<td>
						<?php $option = sc_res_get_option( 'calendar_enabled', SC_RES_DEFAULT_CALENDAR_ENABLED ); ?>
		...
		<div id="metabox_basic_settings_cal1">
		<?php
		$option_use_calendar = $option;
		$option_overlapped   = sc_res_get_option( 'calendar_overlapped', SC_RES_DEFAULT_CALENDAR_OVERLAPPED );
		$calendar_language   = sc_res_get_option( 'calendar_language', SC_RES_DEFAULT_CALENDAR_LANGUAGE );

		if ( $calendar_language == '' ) {
			$calendar_language = sc_res_autodetect_language();
		}

		$calendar_dateformat = sc_res_get_option( 'calendar_dateformat', SC_RES_DEFAULT_CALENDAR_DATEFORMAT );

		if ( sc_res_get_option( 'calendar_dateformat', SC_RES_DEFAULT_CALENDAR_DATEFORMAT ) == 0 ) {
			$dformat_php = 'm/d/Y';
		} else {
			$dformat_php = 'd/m/Y';
		}
		...
		$calendar_mindate = '';
		$value            = sc_res_get_option( 'calendar_mindate', SC_RES_DEFAULT_CALENDAR_MINDATE );
		...
		$calendar_maxdate = '';
		$value            = sc_res_get_option( 'calendar_maxdate', SC_RES_DEFAULT_CALENDAR_MAXDATE );
   ...
   <script>
   var pathCalendar      = "<?php echo cp_sc_res_get_site_url(); ?>/";
   var pathCalendar_full = pathCalendar + "wp-content/plugins/sturtevant-reservations/css/images/corners";
   </script>

   <div id="cal<?php echo CP_BCCF_CALENDAR_ID; ?>" class="rcalendar"><em>Loading calendar data&hellip;</em></div>
   ...
   <script type="text/javascript">
    ...
    (function($) {
        ...
        $calendarjQuery("#cal<?php echo CP_BCCF_CALENDAR_ID; ?>").rcalendar({"calendarId":<?php echo CP_BCCF_CALENDAR_ID; ?>,
                                            "partialDate":<?php echo sc_res_get_option('calendar_mode',SC_RES_DEFAULT_CALENDAR_MODE); ?>,
                                            "edition":true,
                                            ...
                                            "firstDay":<?php echo sc_res_get_option('calendar_weekday', SC_RES_DEFAULT_CALENDAR_WEEKDAY); ?>,
                                            "numberOfMonths":<?php echo sc_res_get_option('calendar_pages',SC_RES_DEFAULT_CALENDAR_PAGES); ?>
                                            });
...
<?php if ($option_use_calendar != 'false') : ?>
   ...
   <table class="form-table" style="width:870px;">
        <tr valign="top">
         <th colspan="4">
           <h3>Settings for both admin and public calendars</h3>
         </th>
        </tr>
        <tr valign="top">
        <td colspan="4">
          <div style="float:left;width:80px;">
            <strong>Cal. Pages:</strong><br />
             <?php $value = sc_res_get_option('calendar_pages',SC_RES_DEFAULT_CALENDAR_PAGES); ?>
...
<?php $v = sc_res_get_option('calendar_language',SC_RES_DEFAULT_CALENDAR_LANGUAGE); ?>
...
<option <?php if ($v == 'cy-GB') echo 'selected'; ?> value="cy-GB">Welsh/UK</option>
          ...
          <div style="float:left;width:120px;">
             ...
             <?php $value = sc_res_get_option('calendar_weekday',SC_RES_DEFAULT_CALENDAR_WEEKDAY); ?>
          ...
          <div style="float:left;width:225px;">
             ...
             <?php $option = sc_res_get_option('calendar_overlapped', SC_RES_DEFAULT_CALENDAR_OVERLAPPED); ?>
       ...
       <tr>
        <td valign="top" colspan="4">
          ...
          <strong>Show cost below calendar?</strong><br />
             <?php $value = sc_res_get_option('calendar_showcost','1'); ?>
           ...
           <div style="width:140px;float:left">
             ...
             <?php $value = sc_res_get_option('calendar_mode',SC_RES_DEFAULT_CALENDAR_MODE); ?>
        ...
        <tr valign="top">
         <th colspan="4">
           <h3>Settings for the public calendar</h3>
         </th>
        </tr>
       ...
       <tr>
        <td width="1%" nowrap valign="top" colspan="2">
         ...
         <input type="text" name="calendar_mindate" size="40" value="<?php echo esc_attr(sc_res_get_option('calendar_mindate',SC_RES_DEFAULT_CALENDAR_MINDATE)); ?>" /><br />
         <em style="font-size:11px;">Examples: 2012-10-25, today, today + 3 days</em>
        </td>
        <td valign="top" colspan="2">
         <strong>Maximum  available date:</strong><br />
         <input type="text" name="calendar_maxdate" size="40" value="<?php echo esc_attr(sc_res_get_option('calendar_maxdate',SC_RES_DEFAULT_CALENDAR_MAXDATE)); ?>" /><br />
         <em style="font-size:11px;">Examples: 2012-10-25, today, today + 3 days</em>
        </td>
        </tr>
       ...
       <tr>
        <td width="1%" nowrap valign="top" colspan="2">
         ...
         <input type="text" name="calendar_minnights" size="40" value="<?php $v = sc_res_get_option('calendar_minnights', '0'); echo esc_attr(($v==''?'0':$v)); ?>" /><br />
        ...
        <td valign="top" colspan="2">
         ...
         <input type="text" name="calendar_maxnights" size="40" value="<?php $v = sc_res_get_option('calendar_maxnights','365'); echo esc_attr(($v==''?'365':$v)); ?>" /><br />
       ...
       <tr>
        <td width="1%" nowrap valign="top" colspan="2">
         ...
         <div id="workingdates">
         <?php $cfmode = sc_res_get_option('calendar_holidaysdays', '1111111'); if ($cfmode == '') $cfmode = '1111111'; ?>
         ...
         <div>
         <?php $cfmode = sc_res_get_option('calendar_startresdays', '1111111'); if ($cfmode == '') $cfmode = '1111111'; ?>
         ...
         <div style="background:#E2EFF8;border: 1px dotted #888888;padding:10px;">
             <div><strong><input type="checkbox" value="1" name="calendar_fixedmode" <?php echo esc_attr((sc_res_get_option('calendar_fixedmode', '')=='1'?'checked="checked"':'')); ?> id="fixedreservation"> Enable Fixed Reservation Length?</strong>
                 <br />&nbsp;&nbsp;&nbsp;&nbsp; <em style="font-size:11px;">Use this for allowing only bookings of a specific number of days.</em>
             </div>
             <div id="container_fixedreservation" <?php echo (sc_res_get_option('calendar_fixedmode', '')=='1'?'':'style="display:none"'); ?>>
                 <br />
                 <?php $v = sc_res_get_option('calendar_fixedreslength','1'); ?>
         ...
         <input type="hidden" name="calendar_holidays" id="holidays" value="<?php echo esc_attr(sc_res_get_option('calendar_holidays','')); ?>" />
         <input type="hidden" name="calendar_startres" id="startreservation" value="<?php echo esc_attr(sc_res_get_option('calendar_startres','')); ?>" />
        </td>
        <td width="1%" nowrap valign="top" colspan="2">
...
<?php else : ?>
    <input type="hidden" name="calendar_language" value="<?php echo esc_attr(sc_res_get_option('calendar_language',SC_RES_DEFAULT_CALENDAR_LANGUAGE)); ?>" />
    <input type="hidden" name="calendar_weekday" value="<?php echo esc_attr(sc_res_get_option('calendar_weekday',SC_RES_DEFAULT_CALENDAR_WEEKDAY)); ?>" />
    <input type="hidden" name="calendar_dateformat" value="<?php echo esc_attr(sc_res_get_option('calendar_dateformat',SC_RES_DEFAULT_CALENDAR_DATEFORMAT)); ?>" />
    <input type="hidden" name="calendar_overlapped" value="<?php echo esc_attr(sc_res_get_option('calendar_overlapped', SC_RES_DEFAULT_CALENDAR_OVERLAPPED)); ?>" />
    <input type="hidden" name="calendar_mode" value="<?php echo esc_attr(sc_res_get_option('calendar_mode',SC_RES_DEFAULT_CALENDAR_MODE)); ?>" />
    <input type="hidden" name="calendar_mindate" value="<?php echo esc_attr(sc_res_get_option('calendar_mindate',SC_RES_DEFAULT_CALENDAR_MINDATE)); ?>" />
    <input type="hidden" name="calendar_maxdate" value="<?php echo esc_attr(sc_res_get_option('calendar_maxdate',SC_RES_DEFAULT_CALENDAR_MAXDATE)); ?>" />
<?php endif; ?>

  <h2>Form Builder</h2>

     <input type="hidden" name="form_structure" id="form_structure" size="180" value="<?php echo str_replace('"','&quot;',str_replace("\r","",str_replace("\n","",esc_attr(sc_res_cleanJSON(sc_res_get_option('form_structure', SC_RES_DEFAULT_form_structure)))))); ?>" />
     <input type="hidden" name="templates" id="templates" value="<?php echo esc_attr( json_encode( sc_res_available_templates() ) ); ?>" />
  ...
  <h2>Submit Button</h2>
     <table class="form-table">
        ...
        <td><input type="text" name="vs_text_submitbtn" size="40" value="<?php $label = esc_attr(sc_res_get_option('vs_text_submitbtn', 'Continue')); echo ($label==''?'Continue':$label); ?>" /></td>
        </tr>
        <tr valign="top">
        <th scope="row">Previous button label (text):</th>
        <td><input type="text" name="vs_text_previousbtn" size="40" value="<?php $label = esc_attr(sc_res_get_option('vs_text_previousbtn', 'Previous')); echo ($label==''?'Previous':$label); ?>" /></td>
        </tr>
        <tr valign="top">
        <th scope="row">Next button label (text):</th>
        <td><input type="text" name="vs_text_nextbtn" size="40" value="<?php $label = esc_attr(sc_res_get_option('vs_text_nextbtn', 'Next')); echo ($label==''?'Next':$label); ?>" /></td>
        </tr>
        <tr valign="top">
  ...
  <h2>Validation Texts</h2>
     <?php $option = sc_res_get_option('vs_use_validation', SC_RES_DEFAULT_vs_use_validation); ?>
     <input type="hidden" name="vs_use_validation" value="<?php echo $option; ?>" />
     <table class="form-table">
        <tr valign="top">
         <td width="1%" nowrap><strong>"is required" text:</strong><br /><input type="text" name="vs_text_is_required" size="40" value="<?php echo esc_attr(sc_res_get_option('vs_text_is_required', SC_RES_DEFAULT_vs_text_is_required)); ?>" /></td>
         <td><strong>"is email" text:</strong><br /><input type="text" name="vs_text_is_email" size="70" value="<?php echo esc_attr(sc_res_get_option('vs_text_is_email', SC_RES_DEFAULT_vs_text_is_email)); ?>" /></td>
        </tr>
        <tr valign="top">
         <td><strong>"is valid captcha" text:</strong><br /><input type="text" name="cv_text_enter_valid_captcha" size="70" value="<?php echo esc_attr(sc_res_get_option('cv_text_enter_valid_captcha', SC_RES_DEFAULT_dexcv_text_enter_valid_captcha)); ?>" /></td>
         <td><strong>"is valid date (mm/dd/yyyy)" text:</strong><br /><input type="text" name="vs_text_datemmddyyyy" size="70" value="<?php echo esc_attr(sc_res_get_option('vs_text_datemmddyyyy', SC_RES_DEFAULT_vs_text_datemmddyyyy)); ?>" /></td>
        </tr>
        <tr valign="top">
         <td><strong>"is valid date (dd/mm/yyyy)" text:</strong><br /><input type="text" name="vs_text_dateddmmyyyy" size="70" value="<?php echo esc_attr(sc_res_get_option('vs_text_dateddmmyyyy', SC_RES_DEFAULT_vs_text_dateddmmyyyy)); ?>" /></td>
         <td><strong>"is number" text:</strong><br /><input type="text" name="vs_text_number" size="70" value="<?php echo esc_attr(sc_res_get_option('vs_text_number', SC_RES_DEFAULT_vs_text_number)); ?>" /></td>
        </tr>
        <tr valign="top">
         <td><strong>"only digits" text:</strong><br /><input type="text" name="vs_text_digits" size="70" value="<?php echo esc_attr(sc_res_get_option('vs_text_digits', SC_RES_DEFAULT_vs_text_digits)); ?>" /></td>
         <td><strong>"under maximum" text:</strong><br /><input type="text" name="vs_text_max" size="70" value="<?php echo esc_attr(sc_res_get_option('vs_text_max', SC_RES_DEFAULT_vs_text_max)); ?>" /></td>
        </tr>
        <tr valign="top">
         <td><strong>"over minimum" text:</strong><br /><input type="text" name="vs_text_min" size="70" value="<?php echo esc_attr(sc_res_get_option('vs_text_min', SC_RES_DEFAULT_vs_text_min)); ?>" /></td>
        </tr>
  ...
  <h2>Price Configuration</h2>
    <table class="form-table">
        ...
        <td><div style="float:left"><input type="text" name="currency" size="3" value="<?php echo esc_attr(sc_res_get_option('currency',SC_RES_DEFAULT_CURRENCY)); ?>" /></div>
        ...
        <th scope="row"><nobr><strong>Total request cost for specific # of days</strong></nobr><br />
          ...
          <?php $option = @intval (sc_res_get_option('max_slots', '0')); if ($option=='') $option = 0;  ?>
       ...
       <tr>
        <td valign="top" colspan="2">
         ...
         <input type="text" name="calendar_suplementminnight" size="40" value="<?php $v = sc_res_get_option('calendar_suplementminnight', '0'); echo esc_attr(($v==''?'0':$v)); ?>" />
         <strong>and</strong>
         <input type="text" name="calendar_suplementmaxnight" size="40" value="<?php $v = sc_res_get_option('calendar_suplementmaxnight', '0'); echo esc_attr(($v==''?'0':$v)); ?>" />
         <strong>nights:</strong>
         <input type="text" name="calendar_suplement" size="40" value="<?php $v = sc_res_get_option('calendar_suplement', '0'); echo esc_attr(($v==''?'0':$v)); ?>" /><br />
       ...
       <tr>
        <td valign="top" colspan="2">
         ...
         <?php $v = sc_res_get_option('calendar_depositenable', '0'); if ($v=='') $v = '0'; ?>
         ...
         <input type="text" name="calendar_depositamount" size="40" value="<?php $v = sc_res_get_option('calendar_depositamount', '0'); echo esc_attr(($v==''?'0':$v)); ?>" />
         &nbsp;&nbsp;
         <?php $v = sc_res_get_option('calendar_deposittype', '0'); if ($v=='') $v = '0'; ?>
  ...
  <h2>Paypal Payment Configuration</h2>
    <table class="form-table">
        ...
        <td><select name="enable_paypal" onchange="bccp_update_pp_payment_selection();">
             <option value="0" <?php if (!sc_res_get_option('enable_paypal',SC_RES_DEFAULT_ENABLE_PAYPAL)) echo 'selected'; ?> >No</option>
             <option value="1" <?php if (sc_res_get_option('enable_paypal',SC_RES_DEFAULT_ENABLE_PAYPAL) == '1') echo 'selected'; ?> >Yes</option>
             <option value="2" <?php if (sc_res_get_option('enable_paypal',SC_RES_DEFAULT_ENABLE_PAYPAL) == '2') echo 'selected'; ?> >Optional</option>
             <option value="3" <?php if (sc_res_get_option('enable_paypal',SC_RES_DEFAULT_ENABLE_PAYPAL) == '3') echo 'selected'; ?> >Use BeanStream.com</option>
            </select>
            ...
            <div id="sc_res_paypal_options_beanstream" style="display:none;margin-top:10px;background:#EEF5FB;border: 1px dotted #888888;padding:10px;width:260px;">
              BeanStream Merchant ID:<br />
              <input type="text" name="enable_beanstream_id" size="40" style="width:250px;" value="<?php echo esc_attr(sc_res_get_option('enable_beanstream_id','')); ?>" />
            </div>

            <div id="sc_res_paypal_options_label" style="display:none;margin-top:10px;background:#EEF5FB;border: 1px dotted #888888;padding:10px;width:260px;">
              Label for the "<strong>Pay with PayPal</strong>" option:<br />
              <input type="text" name="enable_paypal_option_yes" size="40" style="width:250px;" value="<?php echo esc_attr(sc_res_get_option('enable_paypal_option_yes',SC_RES_DEFAULT_PAYPAL_OPTION_YES)); ?>" />
              <br />
              Label for the "<strong>Pay later</strong>" option:<br />
              <input type="text" name="enable_paypal_option_no" size="40" style="width:250px;"  value="<?php echo esc_attr(sc_res_get_option('enable_paypal_option_no',SC_RES_DEFAULT_PAYPAL_OPTION_NO)); ?>" />
            </div>
         </td>
        </tr>
        ...
        <td><input type="text" name="paypal_email" size="40" value="<?php echo esc_attr(sc_res_get_option('paypal_email',SC_RES_DEFAULT_PAYPAL_EMAIL)); ?>" /></td>
        </tr>

        <tr valign="top">
        <th scope="row">Paypal product name</th>
        <td><input type="text" name="paypal_product_name" size="50" value="<?php echo esc_attr(sc_res_get_option('paypal_product_name',SC_RES_DEFAULT_PRODUCT_NAME)); ?>" /></td>
        </tr>

        <tr valign="top">
        <th scope="row">URL to return after successful  payment</th>
        <td><input type="text" name="url_ok" size="70" value="<?php echo esc_attr(sc_res_get_option('url_ok',SC_RES_DEFAULT_OK_URL)); ?>" />
          <br />
        ...
        <td><input type="text" name="url_cancel" size="70" value="<?php echo esc_attr(sc_res_get_option('url_cancel',SC_RES_DEFAULT_CANCEL_URL)); ?>" /></td>
        </tr>


        <tr valign="top">
        <th scope="row">Paypal language</th>
        <td><input type="text" name="paypal_language" value="<?php echo esc_attr(sc_res_get_option('paypal_language',SC_RES_DEFAULT_PAYPAL_LANGUAGE)); ?>" /></td>
        </tr>

        <tr valign="top">
        <th scope="row">Taxes (applied at Paypal)</th>
        <td><input type="text" name="request_taxes" value="<?php echo esc_attr(sc_res_get_option('request_taxes','0')); ?>" /></td>
        </tr>
  ...
  <h2>Optional Services/Items Field</h2>
   <div id="sc_res_optional_services_fields_link">
    <a href="javascript:sc_res_toggle_osf()">Click to show optional services fields configuration...</a>
    <script type="text/javascript">
     function sc_res_toggle_osf()
     {
        document.getElementById("sc_res_optional_services_fields_link").style.display="none";
        document.getElementById("sc_res_optional_services_fields").style.display="";
     }
    </script>
   </div>
   <div id="sc_res_optional_services_fields" style="display:none;">
   <?php for ($k=1;$k<=SC_RES_DEFAULT_SERVICES_FIELDS; $k++) { ?>
    <fieldset style="border: 1px solid #888888;">
     ...
     <table class="form-table">
        ...
        <th scope="row">
         <?php
           $flabel = sc_res_get_option('cp_cal_checkboxes_label'.$k, 'Service');
           if ($flabel == '') $flabel = 'Service';
         ?>
        Field Label: <input type="text" name="cp_cal_checkboxes_label<?php echo $k; ?>" value="<?php echo esc_attr($flabel); ?>" />
        </th>
        </tr>
        <tr valign="top">
        <td colspan="2">
          <strong>If enabled, use the services/items field as:</strong><br />
          <?php $option = sc_res_get_option('cp_cal_checkboxes_type'.$k, SC_RES_DEFAULT_CP_CAL_CHECKBOXES_TYPE); ?>
     ...
     <table class="form-table">
        ...
        <textarea style="width:385px;" wrap="off" rows="4" name="cp_cal_checkboxes<?php echo $k; ?>"><?php echo sc_res_get_option('cp_cal_checkboxes'.$k, SC_RES_DEFAULT_CP_CAL_CHECKBOXES); ?></textarea>
        </th>
        <td>
        <em>Note: This is an optional field that appears only if some option is specified.</em>
        <br /><u><strong>Sample Format:</strong></u><br />
        <?php echo str_replace("\n", "<br />", SC_RES_DEFAULT_EXPLAIN_CP_CAL_CHECKBOXES); ?></td>
        </tr>
     </table>
    </fieldset>
   <?php } ?>
  ...
  <h2>Notification Settings to Administrator(s)</h2>
     <table class="form-table">
        <tr valign="top">
        <th scope="row">Notification "from" email</th>
        <td><input type="text" name="notification_from_email" size="40" value="<?php echo esc_attr(sc_res_get_option('notification_from_email', SC_RES_DEFAULT_PAYPAL_EMAIL)); ?>" /></td>
        </tr>
        <tr valign="top">
        <th scope="row">Send notification to email</th>
        <td><input type="text" name="notification_destination_email" size="40" value="<?php echo esc_attr(sc_res_get_option('notification_destination_email', SC_RES_DEFAULT_PAYPAL_EMAIL)); ?>" />
            <br />
        ...
        <td><input type="text" name="email_subject_notification_to_admin" size="70" value="<?php echo esc_attr(sc_res_get_option('email_subject_notification_to_admin', SC_RES_DEFAULT_SUBJECT_NOTIFICATION_EMAIL)); ?>" /></td>
        </tr>
        <tr valign="top">
        <th scope="row">Email format?</th>
        <td>
          <?php $option = sc_res_get_option('notification_emailformat', 'text'); ?>
        ...
        <td><textarea cols="70" rows="5" name="email_notification_to_admin"><?php echo sc_res_get_option('email_notification_to_admin', SC_RES_DEFAULT_NOTIFICATION_EMAIL); ?></textarea></td>
        </tr>
     </table>
  <h2>Email Copy to User</h2>
     <table class="form-table">
        ...
        <td><select id="cu_user_email_field" name="cu_user_email_field" def="<?php echo esc_attr(sc_res_get_option('cu_user_email_field', SC_RES_DEFAULT_cu_user_email_field)); ?>"></select></td>
        </tr>
        <tr valign="top">
        <th scope="row">Email subject confirmation to user</th>
        <td><input type="text" name="email_subject_confirmation_to_user" size="70" value="<?php echo esc_attr(sc_res_get_option('email_subject_confirmation_to_user', SC_RES_DEFAULT_SUBJECT_CONFIRMATION_EMAIL)); ?>" /></td>
        </tr>
        <tr valign="top">
        <th scope="row">Email format?</th>
        <td>
          <?php $option = sc_res_get_option('copyuser_emailformat', 'text'); ?>
        ...
        <td><textarea cols="70" rows="5" name="email_confirmation_to_user"><?php echo sc_res_get_option('email_confirmation_to_user', SC_RES_DEFAULT_CONFIRMATION_EMAIL); ?></textarea></td>
        </tr>
     </table>
  <h3 class='hndle' style="padding:5px;"><span>Reminder Settings</span></h3>
     <table class="form-table">
        ...
        <td><input type="checkbox" name="enable_reminder" id="enable_reminder" onclick="sc_res_checkreminderstatus();" size="40" value="1" <?php if (sc_res_get_option('enable_reminder',0)) echo 'checked'; ?> /></td>
        </tr>
     </table>
     <table class="form-table" id="sc_res_remindertable">
        <tr><td colspan="2"><hr /></td></tr>
        <tr valign="top">
        <th scope="row">Send reminder:</th>
        <td><input type="text" name="reminder_hours" size="2" value="<?php echo esc_attr(sc_res_get_option('reminder_hours', 72)); ?>" /> hours <strong>BEFORE</strong> the booking
        ...
        <td><input type="text" name="reminder_subject" size="70" value="<?php echo esc_attr(sc_res_get_option('reminder_subject', 'Booking reminder...')); ?>" /></td>
        </tr>
        <tr valign="top">
        <th scope="row">Email format?</th>
        <td>
          <?php $option = sc_res_get_option('nremind_emailformat', 'text'); ?>
        ...
        <td><textarea cols="70" rows="5" name="reminder_content"><?php echo sc_res_get_option('reminder_content', SC_RES_DEFAULT_REMINDER_CONTENT); ?></textarea></td>
        </tr>
     </table>
     <table class="form-table" id="sc_res_remindertable2">
        <tr><td colspan="2"><hr /></td></tr>
        <tr valign="top">
        <th scope="row">Send reminder:</th>
        <td><input type="text" name="reminder_hours2" size="2" value="<?php echo esc_attr(sc_res_get_option('reminder_hours2', 72)); ?>" /> hours <strong>AFTER</strong> the booking
        <br /><em>Note: Hours date based in server time. Server time now is <?php echo date("Y-m-d H:i"); ?></em>
        </td>
        </tr>
        <tr valign="top">
        <th scope="row">Reminder email subject</th>
        <td><input type="text" name="reminder_subject2" size="70" value="<?php echo esc_attr(sc_res_get_option('reminder_subject2', 'Thank you for your booking...')); ?>" /></td>
        </tr>
        <tr valign="top">
        <th scope="row">Email format?</th>
        <td>
          <?php $option = sc_res_get_option('nremind_emailformat2', 'text'); ?>
        ...
        <tr valign="top">
        <th scope="row">Reminder email message</th>
        <td><textarea cols="70" rows="5" name="reminder_content2"><?php echo sc_res_get_option('reminder_content2', SC_RES_DEFAULT_REMINDER_CONTENT_AFTER); ?></textarea></td>
        </tr>
     </table>
     <script type="text/javascript">
       function sc_res_checkreminderstatus() {
            if (document.getElementById("enable_reminder").checked)
            {
                document.getElementById("sc_res_remindertable").style.display = '';
                document.getElementById("sc_res_remindertable2").style.display = '';
            }
            else
            {
                document.getElementById("sc_res_remindertable").style.display = 'none';
                document.getElementById("sc_res_remindertable2").style.display = 'none';
            }
       }
       sc_res_checkreminderstatus();
     </script>
  <h2>Captcha Verification</h2>
     <table class="form-table">
        ...
        <td colspan="5">
          <?php $option = sc_res_get_option('dexcv_enable_captcha', TDE_BCCFDEFAULT_dexcv_enable_captcha); ?>
        ...
        <tr valign="top">
         ...
         <td><input type="text" name="dexcv_width" size="10" value="<?php echo esc_attr(sc_res_get_option('dexcv_width', TDE_BCCFDEFAULT_dexcv_width)); ?>"  onblur="generateCaptcha();"  /></td>
         <th scope="row">Height:</th>
         <td><input type="text" name="dexcv_height" size="10" value="<?php echo esc_attr(sc_res_get_option('dexcv_height', TDE_BCCFDEFAULT_dexcv_height)); ?>" onblur="generateCaptcha();"  /></td>
         <th scope="row">Chars:</th>
         <td><input type="text" name="dexcv_chars" size="10" value="<?php echo esc_attr(sc_res_get_option('dexcv_chars', TDE_BCCFDEFAULT_dexcv_chars)); ?>" onblur="generateCaptcha();"  /></td>
        </tr>

        <tr valign="top">
         <th scope="row">Min font size:</th>
         <td><input type="text" name="dexcv_min_font_size" size="10" value="<?php echo esc_attr(sc_res_get_option('dexcv_min_font_size', TDE_BCCFDEFAULT_dexcv_min_font_size)); ?>" onblur="generateCaptcha();"  /></td>
         <th scope="row">Max font size:</th>
         <td><input type="text" name="dexcv_max_font_size" size="10" value="<?php echo esc_attr(sc_res_get_option('dexcv_max_font_size', TDE_BCCFDEFAULT_dexcv_max_font_size)); ?>" onblur="generateCaptcha();"  /></td>
        ...
        <tr valign="top">
         ...
         <td><input type="text" name="dexcv_noise" size="10" value="<?php echo esc_attr(sc_res_get_option('dexcv_noise', TDE_BCCFDEFAULT_dexcv_noise)); ?>" onblur="generateCaptcha();" /></td>
         <th scope="row">Noise Length:</th>
         <td><input type="text" name="dexcv_noise_length" size="10" value="<?php echo esc_attr(sc_res_get_option('dexcv_noise_length', TDE_BCCFDEFAULT_dexcv_noise_length)); ?>" onblur="generateCaptcha();" /></td>
        </tr>


        <tr valign="top">
         <th scope="row">Background:</th>
         <td><input type="text" name="dexcv_background" size="10" value="<?php echo esc_attr(sc_res_get_option('dexcv_background', TDE_BCCFDEFAULT_dexcv_background)); ?>" onblur="generateCaptcha();" /></td>
         <th scope="row">Border:</th>
         <td><input type="text" name="dexcv_border" size="10" value="<?php echo esc_attr(sc_res_get_option('dexcv_border', TDE_BCCFDEFAULT_dexcv_border)); ?>" onblur="generateCaptcha();" /></td>
        </tr>
        ...
        <tr valign="top">
         ...
         <td>
            <select name="dexcv_font" onchange="generateCaptcha();" >
              <option value="font-1.ttf"<?php if ("font-1.ttf" == sc_res_get_option('dexcv_font', TDE_BCCFDEFAULT_dexcv_font)) echo " selected"; ?>>Font 1</option>
              <option value="font-2.ttf"<?php if ("font-2.ttf" == sc_res_get_option('dexcv_font', TDE_BCCFDEFAULT_dexcv_font)) echo " selected"; ?>>Font 2</option>
              <option value="font-3.ttf"<?php if ("font-3.ttf" == sc_res_get_option('dexcv_font', TDE_BCCFDEFAULT_dexcv_font)) echo " selected"; ?>>Font 3</option>
              <option value="font-4.ttf"<?php if ("font-4.ttf" == sc_res_get_option('dexcv_font', TDE_BCCFDEFAULT_dexcv_font)) echo " selected"; ?>>Font 4</option>
            </select>
         </td>
        </tr>
 ...
 <?php
	global $sc_res_addons_objs_list, $sc_res_addons_active_list;
	if( count( $sc_res_addons_active_list ) )
	{
		_e( '<h2>Add-Ons Settings:</h2><hr />', 'sc-res' );
		foreach( $sc_res_addons_active_list as $addon_id ) if( isset( $sc_res_addons_objs_list[ $addon_id ] ) ) print $sc_res_addons_objs_list[ $addon_id ]->get_addon_form_settings( CP_CONTACTFORMPP_ID );
	}
 ?>
    ...
    <h2>Note</h2>
     ...
     <?php print '<img hspace="5" src="'.plugins_url('/images/dex_apps.gif', __FILE__).'" alt="'.__('Insert Booking Calendar','sc-res').'" />';     ?>
...
<script type="text/javascript">
 ...
 $j('#dex_nocodes_availmsg').load('<?php echo cp_sc_res_get_site_url(true); ?>/?dex_bccf=loadcoupons&inAdmin=1&dex_item=<?php echo CP_BCCF_CALENDAR_ID; ?>');
 $j('#dex_dc_subccode').click (function() {
                               ...
                               $j('#dex_nocodes_availmsg').load('<?php echo cp_sc_res_get_site_url(true); ?>/?dex_bccf=loadcoupons&inAdmin=1&dex_item=<?php echo CP_BCCF_CALENDAR_ID; ?>'+params);
                               $j('#dex_dc_code').val();
                             });
  function dex_delete_coupon(id)
  {
     $j('#dex_nocodes_availmsg').load('<?php echo cp_sc_res_get_site_url(true); ?>/?dex_bccf=loadcoupons&inAdmin=1&dex_item=<?php echo CP_BCCF_CALENDAR_ID; ?>&delete=1&code='+id);
  }
  $j(function() {
  ...
  $j('#dex_noseasons_availmsg').load('<?php echo cp_sc_res_get_site_url(true); ?>/?dex_bccf=loadseasonprices&inAdmin=1&dex_item=<?php echo CP_BCCF_CALENDAR_ID; ?>');
  $j('#dex_dc_subcseasons').click (function() {
                               ...
                               $j('#dex_noseasons_availmsg').load('<?php echo cp_sc_res_get_site_url(true); ?>/?dex_bccf=loadseasonprices&inAdmin=1&dex_item=<?php echo CP_BCCF_CALENDAR_ID; ?>'+params);
                               $j('#dex_dc_price').val();
                             });
  function dex_delete_season_price(id)
  {
     $j('#dex_noseasons_availmsg').load('<?php echo cp_sc_res_get_site_url(true); ?>/?dex_bccf=loadseasonprices&inAdmin=1&dex_item=<?php echo CP_BCCF_CALENDAR_ID; ?>&delete=1&code='+id);
  }
  ...
  function bccp_update_pp_payment_selection()
  {
     ...
     if (ppoption == '2')
         document.getElementById("sc_res_paypal_options_label").style.display = "";
     else
         document.getElementById("sc_res_paypal_options_label").style.display = "none";
     if (ppoption == '3')
         document.getElementById("sc_res_paypal_options_beanstream").style.display = "";
     else
         document.getElementById("sc_res_paypal_options_beanstream").style.display = "none";
