<?php
/**
 * The reservations data source file.
 *
...
add_action( 'init', 'bccf_init_ds', 0 );

function bccf_init_ds() {
		...
		switch( $_REQUEST['cffaction'] ) {
			...
			case 'test_db_query' :
				...
				if ( $_REQUEST['active'] == 'structure' ) {
					_bccf_function_check_for_variable( $_REQUEST['table'] );
					_bccf_function_check_for_variable( $_REQUEST['where'] );
				} else {
					_bccf_function_check_for_variable( $_REQUEST['query'] );
				}
			...
			case 'get_post_types':
				print json_encode( get_post_types( [ 'public' => true ] ) );
				exit;
			break;
			...
			case 'get_posts':
				...
				$result_obj = new stdClass;
				$result_obj->data = bccf_ds( $_REQUEST );
				print( json_encode( $result_obj ) );
			...
			case 'get_available_taxonomies':
				print json_encode( get_taxonomies( [ 'public' => true ], 'objects' ) );
				exit;
			break;
 ...
 * @return void
 */
function bccf_ds( $data ) {

	switch( $data['data_source'] ) {

		case 'database':
			return bccf_ds_db( $data );
		break;

		case 'csv':
			return bccf_ds_csv( $data );
		break;

		case 'post_type':
			return bccf_ds_post_type( $data );
		break;

		case 'taxonomy':
			return bccf_ds_taxonomy( $data );
		break;

		case 'user':
			return bccf_ds_user( $data );
		break;
 ...
 * @return void
 */
function _bccf_function_check_for_variable( $str ) {

	if ( preg_match( '/<%fieldname\d+%>/', $str ) ) {
		print __( 'Your query includes variables, so it cannot be tested from the form\'s edition', 'sc-res' );
		exit;
	}
 ...
 * @return void
 */
function _bccf_function_set_attr( &$obj, $attr, $arr, $elem ) {
 ...
 * @return void
 */
function bccf_ds_db( $data ) {

	global $wpdb, $cpcff_db_connect, $cp_plugin;
	...
	if ( ! empty( $data['connection'] ) && ! empty( $data['form'] ) ) {
		...
		$connection_data = unserialize( mcrypt_decrypt( MCRYPT_RIJNDAEL_256, dex_bccf_get_option( 'form_structure', DEX_BCCF_DEFAULT_form_structure, $data['form'] ), base64_decode( $data['connection'] ), MCRYPT_MODE_ECB ) );
...
function bccf_ds_csv( $data ){}

function bccf_ds_post_type( $data ) {
	...
	if ( ! empty( $data['id'] ) ) {
		...
		if ( ! is_null( $result ) ) {
			...
			_bccf_function_set_attr( $tmp, 'value', $result, $data['value'] );
			_bccf_function_set_attr( $tmp, 'text',  $result, $data['text'] );
 ...
 * @return void
 */
function bccf_ds_taxonomy( $data ) {
	...
	if ( ! empty( $data['id'] ) || !empty( $data['slug'] ) ) {
		...
		if ( ! empty( $data[ 'taxonomy' ] ) ) {
			...
			$results = get_terms( $data['taxonomy'], [ 'hide_empty' => 0 ] );
...
function bccf_ds_user( $data ) {
	...
	if ( ! empty( $data['logged'] ) && $data['logged'] !== 'false' ) {
		...
		if ( ! empty( $result ) ) {
			...
			_bccf_function_set_attr( $tmp, 'value', $result->data, $data['value'] );
		...
		foreach( $results as $result ) {
			$tmp = new stdClass;
			_bccf_function_set_attr( $tmp, 'value', $result->data, $data['value'] );
			_bccf_function_set_attr( $tmp, 'text', $result->data, $data['text'] );
			$users[] = $tmp;
		}
