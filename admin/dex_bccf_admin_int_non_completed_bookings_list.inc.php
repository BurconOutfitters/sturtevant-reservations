...
if ( isset( $_GET['ld'] ) && $_GET['ld'] != '' ) {
    ...
    $wpdb->query( 'DELETE FROM `'.SC_RES_TABLE_NAME.'` WHERE id=' . intval( $_GET['ld'] ) );
    $message = __( 'Item deleted', 'sc-res' );

} elseif ( isset( $_GET['lup'] ) && $_GET['lup'] != '' ) {

    $wpdb->query( "UPDATE `".SC_RES_TABLE_NAME."` SET status='".esc_sql( $_GET['paid'] )."' WHERE id=" . intval( $_GET['lup'] ) );
...
$buffer_events   = '0';
$completedevents = $wpdb->get_results( "SELECT * FROM " . SC_RES_CALENDARS_TABLE_NAME . " WHERE reservation_calendar_id=" . CP_BCCF_CALENDAR_ID );
foreach ( $completedevents as $item ) {
    $buffer_events .= ',' . $item->reference;
}

//left join ".SC_RES_CALENDARS_TABLE_NAME." on ".SC_RES_CALENDARS_TABLE_NAME.".reference=".SC_RES_TABLE_NAME.".id
$events = $wpdb->get_results( "SELECT * FROM " . SC_RES_TABLE_NAME . " WHERE calendar=" . CP_BCCF_CALENDAR_ID . $cond . " AND NOT id in ( $buffer_events ) ORDER BY booked_time_unformatted_s DESC" );
...
<script type="text/javascript">
 ...
 function cp_updatePaid( id, paid ) {
    document.location = 'admin.php?page=reservations&cal=<?php echo $_GET['cal']; ?>&lists=1&paid=' + paid + '&lup=' + id + '&r=' + Math.random();
 }
</script>
<div class="wrap reservations reservation-submissions-inc">
    ...
    <p>
        ...
        <input class="button" type="button" name="noncbtn" value="<?php _e( 'Completed Submissions', 'sc-res' ); ?>" onclick="document.location='admin.php?page=reservations&cal=<?php echo $_GET['cal']; ?>&list=1';">
    </p>
    <hr />
