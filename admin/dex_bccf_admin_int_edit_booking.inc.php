...
if ( true ) :   // (cp_sc_res_is_administrator() || $mycalendarrows[0]->conwer == $current_user->ID) {
    ...
    if ( $event->reference != '' ) {
        $form_data = json_decode( sc_res_cleanJSON( sc_res_get_option( 'form_structure', SC_RES_DEFAULT_form_structure ) ) );

        $org_booking = $wpdb->get_results( "SELECT buffered_date FROM " . SC_RES_TABLE_NAME . " WHERE id=" . $event->reference );
    ...
    if (count($_POST) > 0) {
        ...
        $dfoption   = sc_res_get_option( 'calendar_dateformat', SC_RES_DEFAULT_CALENDAR_DATEFORMAT );
        if ( $dfoption == '0' ) {
            $format = 'm/d/Y' . $format;
        } else {
            $format = 'd/m/Y' . $format;
        ...
        $description = __( 'Form: ', 'sc-res' ) . sc_res_get_option( 'uname', '' ) . '<br />' . __( 'Date From-To: ', 'sc-res' ) . date( $format, strtotime( $datatime_s ) ) . '-' . date( $format, strtotime( $datatime_e ) ) . '<br />';
        ...
        foreach ( $params_new as $item => $value) {
            if ( $value != '' && $item != 'startdate' && $item != 'enddate'
                ...
                $name         = sc_res_get_field_name( $item, $form_data[0] );
        ...
        if ( $event->reference != '' ) {
            $wpdb->update ( SC_RES_TABLE_NAME, $data2, [ 'id' => $event->reference ] );
        }
...
<div class="wrap reservations reservation-submissions-edit">
    ...
    <form method="post" name="dexeditfrm" action="">
        <h3><?php _e( 'Submission Data', 'sc-res' ); ?></h3>
        <table class="form-table">
            ...
            <tr valign="top">
                <th scope="row"><?php _e( 'Booking Title', 'sc-res' ); ?></th>
                <td><input type="text" name="title" size="40" value="<?php echo esc_attr( $event->title ); ?>" /></td>
            </tr>
            <?php foreach ( $params as $item => $value ) { ?>
            <tr valign="top">
                <th scope="row"><?php $name = sc_res_get_field_name( $item, $form_data[0] ); echo $name; ?></th>
                <td>
                    <?php if ( strpos( $value, "\n" ) > 0 || strlen( $value ) > 80 ) { ?>
        ...
        <p class="submit">
            <input class="button button-primary" type="submit" name="submit" id="submit" class="button-primary" value="<?php _e( 'Save', 'sc-res' ); ?>"  /> <input type="button" value="<?php _e( 'Cancel', 'sc-res' ); ?>" onclick="javascript:gobackapp();">
        </p>
    </form>
</div>
