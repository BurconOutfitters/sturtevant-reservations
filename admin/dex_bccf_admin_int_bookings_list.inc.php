...
if ( isset( $_GET['ld'] ) && $_GET['ld'] != '' ) {
    ...
    $wpdb->query( 'DELETE FROM `'.SC_RES_CALENDARS_TABLE_NAME.'` WHERE id=' . intval( $_GET['ld'] ) );
    $message = __( 'Form deleted', 'sc-res' );

} elseif ( isset( $_GET['lup'] ) && $_GET['lup'] != '' ) {

    $wpdb->query( "UPDATE `".SC_RES_CALENDARS_TABLE_NAME."` SET status='".$_GET['paid']."' WHERE id=" . intval( $_GET['lup'] ) );
...
$events_query = "SELECT * FROM " . SC_RES_CALENDARS_TABLE_NAME . " WHERE reservation_calendar_id=" . CP_BCCF_CALENDAR_ID . $cond . " ORDER BY datatime_s DESC";

/**
 * Allows modify the query of messages, passing the query as parameter
 * returns the new query
 */
$events_query            = apply_filters( 'sc_res_messages_query', $events_query );
$events                  = $wpdb->get_results( $events_query );
$total_pages             = ceil( count( $events ) / $records_per_page );
$option_calendar_enabled = sc_res_get_option( 'calendar_enabled', SC_RES_DEFAULT_CALENDAR_ENABLED );
?>
<script type="text/javascript">
    function cp_deleteMessageItem( id ) {
        if ( confirm( '<?php _e( 'Are you sure that you want to delete this reservation?', 'sc-res' ); ?>' ) ) {
            document.location = 'admin.php?page=reservations&cal=<?php echo $_GET['cal']; ?>&list=1&ld=' + id + '&r=' + Math.random();
        }
    }

    function cp_editItem( id, cal ) {
        document.location = 'admin.php?page=reservations&cal=' + cal + '&edit=' + id + '&r=' + Math.random();
    }

    function cp_updatePaid( id,paid ) {
        document.location = 'admin.php?page=reservations&cal=<?php echo $_GET['cal']; ?>&list=1&paid=' + paid + '&lup=' + id + '&r=' + Math.random();
    }
</script>
<div class="wrap reservations reservation-submissions">
    ...
    <p>
        ...
        <input class="button" type="button" name="noncbtn" value="<?php _e( 'Forms Not Completed', 'sc-res' ); ?>" onclick="document.location='admin.php?page=reservations&cal=<?php echo $_GET["cal"]; ?>&list2=1';">
    </p>
    <hr />
    ...
    <form action="admin.php" method="get">
        ...
        <span class="submit"><input class="button" type="submit" name="sc_res_appointments_csv" value="<?php _e( 'Export to CSV', 'sc-res' ); ?>" /></span>
    </form>
    <!-- End list form -->
    <br />
...
<?php
// Show a message if the current user is not allowed acces to the selected calendar.
else : ?>
    <br />
